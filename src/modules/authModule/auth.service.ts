...
import { IAuthServices, Events, TokenType, IUser } from "../../common";
...
class AuthServices implements IAuthServices {
  ...
  signup = async (
    ...
    try {
      const { firstName, lastName, email, password, approved }: signupDTO =
        req.body;
  ...
  login = async (
    ...
    try {
      ...
      const { accessToken, refreshToken } = generateToken({
        payload: { id: user._id, approved: user.approved },
      });
      return successHandler({
        res,
  ...
  updateEmail = async (
    ...
    try {
      ...
      return successHandler({
        res,
        status: 200,
        msg: "Email update OTP sent successfully",
      });
    } catch (error) {
      throw error;
    }
